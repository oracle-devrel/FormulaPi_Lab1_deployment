...
module "grassrootsAPEX_vcn" {
  source = "oracle-terraform-modules/vcn/oci"

  # general oci parameters
  compartment_id = var.compartment_ocid
  vcn_name = "grassrootsAPEX_vcn"
  ...
  subnets = {
    grassrootsAPEX_public_vcn = {
      name = "grassrootsAPEX_public_subnet"
      type = "public"
...
#   compartment_id = var.compartment_ocid
#   vcn_id           = module.grassrootsAPEX_vcn.vcn_id
#   display_name   = "Data-Ingestion_SL_UDP"
#   timeouts {
#     create = "60m"
#   }
...
module "grassrootsAPEX" {
  ...
  subnet_ocids         = [module.grassrootsAPEX_vcn.subnet_id["grassrootsAPEX_public_subnet"]]
  # network_security_group_id = oci_core_network_security_group.data_ingestion_nsg.id
...
resource null_resource export_file_wallet {
  ...
  connection {
    agent       = false
    timeout     = "30m"
    host        = module.grassrootsAPEX.public_ip_all_attributes[0]["ip_address"]
    user        = "opc"
